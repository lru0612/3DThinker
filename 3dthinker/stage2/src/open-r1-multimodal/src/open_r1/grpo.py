...
def main(script_args, training_args, model_args):
    ...
    # QUESTION_TEMPLATE = "{Question} 请你先输出思考过程，思考过程需用<think></think>标签包裹。在思考过程里，使用<area></area>标签表示关注的边界框，<text></text>标签中进行文本分析思考，并且让这两种标签交替使用，从而逐步思考与纠正，进而定位到关键区域。最后，在<answer></answer>标签中输出最终答案。"
    # QUESTION_TEMPLATE = "{Question} 请直接输出是否违规的标签：'是'或'否'，不要输出思考过程。"
