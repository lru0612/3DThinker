...
def extract_embeddings_between_ids_default(predict_embeddings, output_ids, latent_size, start_id=151652, end_id=151653):
    ...
    如果没找到，则使用备用token id [151644, 77091] 之后开始提取latent_size个token
    
    Args:
        predict_embeddings: torch.Tensor, shape [1, 2223, 2048]
        output_ids: torch.Tensor, shape [1, 2224]
        latent_size: int, 要提取的embedding数量
    ...
    # 检查是否找到了原始的start_id和end_id
    if len(start_positions) == 0 or len(end_positions) == 0:
        return extract_with_fallback_ids(predict_embeddings, ids, latent_size, device)
...
def extract_with_fallback_ids(predict_embeddings, ids, latent_size, device):
    """
    使用备用token id [151644, 77091] 来提取embeddings
    
    Args:
        predict_embeddings: torch.Tensor, shape [1, 2223, 2048]
        ids: torch.Tensor, shape [2224]
        latent_size: int, 要提取的embedding数量
    ...
    fallback_ids = [151644, 77091]
    fallback_tensors = [torch.tensor(fid, device=device) for fid in fallback_ids]
    
    # 寻找备用ID序列的位置
    for i in range(len(ids) - len(fallback_ids) + 1):
        # 检查是否匹配整个序列
        if all(ids[i + j] == fallback_tensors[j] for j in range(len(fallback_ids))):
            # 找到了备用ID序列，从序列结束后开始提取
            sequence_end_idx = i + len(fallback_ids)  # 序列结束位置的下一个位置
            
            max_embed_idx = predict_embeddings.shape[1] - 1  # 2222
            
            # 确保提取范围不超出embedding的边界
            embed_start_idx = min(sequence_end_idx, max_embed_idx)
            embed_end_idx = min(embed_start_idx + latent_size, max_embed_idx + 1)
            
            # 如果超出边界，调整提取范围
            if embed_end_idx > predict_embeddings.shape[1]:
                embed_end_idx = predict_embeddings.shape[1]
                embed_start_idx = max(0, embed_end_idx - latent_size)
            
            extracted_embeddings = predict_embeddings[:, embed_start_idx:embed_end_idx, :]
            
            return extracted_embeddings, embed_start_idx, embed_end_idx
    
    # 如果连备用ID序列也没找到
    print(f"未找到备用ID序列 {fallback_ids}")
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ## TODO
        processing_class.tokenizer.add_tokens("<|latent_pad|>", special_tokens=True)
        processing_class.tokenizer.add_tokens("<|latent_start|>", special_tokens=True)
        processing_class.tokenizer.add_tokens("<|latent_end|>", special_tokens=True)
        self.latent_start_idx = processing_class.tokenizer("<|latent_start|>", return_tensors="pt")["input_ids"][0] # 151666
        self.latent_end_idx = processing_class.tokenizer("<|latent_end|>", return_tensors="pt")["input_ids"][0] # 151667
    ...
    # def place_input_image(self, text, image_pad="<|vision_start|><|image_pad|><|vision_end|>", image_placeholder="<image>", sep_token="<|im_start|>assistant") -> str:
    ...
    #             try:
    #                 # 使用与推理代码相同的resize逻辑
    #                 # resized_image = self.resize_image_proportionally(img)
    ...
    #             # 如果是conversational格式，取最后一个用户消息
    #             if isinstance(prompt_text[0], dict) and "content" in prompt_text[0]:
    #                 last_content = prompt_text[-1]["content"]
    #                 if isinstance(last_content, list):
    #                     # 提取文本部分
    #                     text_parts = [item["text"] for item in last_content if item.get("type") == "text"]
    #                     prompt_text = "".join(text_parts)
    #                 else:
    #                     prompt_text = last_content
            
    #         # 构建图像提示符
    #         image_prompt = "<image>" * len(imgs)
    #         content_list.append({'type': 'text', 'text': image_prompt + prompt_text})
            
    #         conversations_list.append([{'role': 'user', 'content': content_list}])

    #     # 使用与推理代码相同的处理方式
    #     texts = [self.processing_class.apply_chat_template(conv, tokenize=False) for conv in conversations_list]
    #     texts = [self.place_input_image(text, sep_token=None) for text in texts]
    ...
    #     prompt_ids, prompt_mask = prompt_inputs["input_ids"], prompt_inputs["attention_mask"]
        ...
        with torch.no_grad():
            ...
            predict_embeddings = outputs.hidden_states[-1]
            # 创建图像token的掩码 (假设151655是图像token的ID)
            mask = (prompt_completion_ids == 151655).int()
            ...
            ## TODO:latent
            extracted_emb, start_pos, end_pos = extract_embeddings_between_ids_default(predict_embeddings, prompt_completion_ids, latent_size=12, start_id=self.latent_start_idx, end_id=self.latent_end_idx)
            # print(image_embeddings.shape) # torch.Size([1, 4, 391, 2048])
            # print(extracted_emb.shape) # torch.Size([1, 12, 2048])
            ...
            # # 将所有隐藏状态连接起来
            # predict_embeddings = torch.cat([torch.cat(step_hidden_states, dim=0).unsqueeze(1) 
            #                             for step_hidden_states in generated_hidden_states], dim=1)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # print(image_embeddings.shape) # torch.Size([1, 4, 391, 2048])
                # print(extracted_emb.shape) # torch.Size([1, 12, 2048])
                output_reward_func = reward_func(prompts=prompts, completions=completions, image_embeddings=image_embeddings,extracted_emb=extracted_emb,**reward_kwargs)
